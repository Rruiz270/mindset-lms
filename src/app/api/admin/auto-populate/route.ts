...
// Load all students from CSV data file
function loadStudentData() {
  try {
    const studentsDataPath = path.join(process.cwd(), 'students_data.js');
    const studentsData = fs.readFileSync(studentsDataPath, 'utf-8');
    const allStudents = eval(studentsData.replace('const allStudents = ', '')).filter((s: any) => s.email && s.name);
    return allStudents;
  } catch (error) {
  ...
  try {
    ...
    if (allStudents.length === 0) {
      return NextResponse.json({
        ...
        details: 'Unable to load students from data file'
      }, { status: 400 });
    }
