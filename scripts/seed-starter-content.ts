// This script seeds all Starter level content
// Run with: npm run seed:starter
...
async function seedStarterContent() {
  ...
  try {
    ...
    console.log(`Found ${topics.length} Starter topics`)
    ...
    for (const topic of topics) {
      ...
      if (!topicData) {
        console.log(`⚠️  No content defined for topic: ${topic.name}`)
        continue
      }
    ...
    console.log('\n✨ Seeding complete!')
    console.log(`📊 Created ${totalContent} content items and ${totalExercises} exercises`)
